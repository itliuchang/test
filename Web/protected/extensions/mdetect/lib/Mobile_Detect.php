...
class Mobile_Detect
{
    ...
    protected static $properties = array(
        ...
        'WindowsPhoneOS' => array( 'Windows Phone OS [VER]', 'Windows Phone [VER]'),
        'Windows Phone'    => 'Windows Phone [VER]',
        'Windows CE'       => 'Windows CE/[VER]',
        // http://social.msdn.microsoft.com/Forums/en-US/windowsdeveloperpreviewgeneral/thread/6be392da-4d2f-41b4-8354-8dcee20c85cd
        'Windows NT'       => 'Windows NT [VER]',
        'Symbian'          => array('SymbianOS/[VER]', 'Symbian/[VER]'),
        'SymbianOS'          => array('SymbianOS/[VER]', 'Symbian/[VER]'),
    ...
    private function prepareVersion($ver, $limit = 4, $type = self::VERSION_TYPE_STRING){
        if(empty($ver)) return '';
        $ver = str_replace(array('_', ' ', '/'), '.', $ver);
        $arrVer = explode('.', $ver, $limit);

        if(isset($arrVer[$limit - 1])){
            $arrVer[$limit - 1] = @str_replace('.', '', $arrVer[$limit - 1]);
        }
    ...
    public function getOS(){
        $res = array('title' => '', 'version' => '');
        foreach($this->getOperatingSystems() as $os => $regex){
            if(empty($regex)) continue;
            $regex = str_replace('/', '\/', $regex);
            if(preg_match('/'.$regex.'/is', $this->getUserAgent())){
                $res['version'] = $this->prepareVersion(trim($this->version($os) ?: ''));
                $res['title'] = strtolower($os) ?: '';
                break;
            } 
        }
        if(!in_array($res['title'], array('ios', 'javaos', 'webos', 'palmos'))){
            // $res['title'] = preg_replace('/(android)os$/i', '\\1', $res['title']);
            $res['title'] = preg_replace('/os$/i', '\\1', $res['title']);
        }
        return $res;
    }
    ...
    public function getBrowser(){
        $res = array('title' => '', 'version' => '');
        foreach($this->getBrowsers() as $browser => $regex){
            if(empty($regex)) continue;
            $regex = str_replace('/', '\/', $regex);
            if(preg_match('/'.$regex.'/is', $this->getUserAgent())){
                $res['version'] =  $this->prepareVersion(trim($this->version($browser) ?: ''));
                $res['title'] = strtolower($browser) ?: '';
                break;
            }
        }
        return $res;
    ...
    public function getDevice(){
        $res = array('title' => '', 'version' => '');
        $all_devices = array_merge($this->getPhoneDevices(), $this->getTabletDevices());
        foreach($all_devices as $device => $regex){
            if(empty($regex)) continue;
            $regex = str_replace('/', '\/', $regex);
            if(preg_match('/'.$regex.'/is', $this->getUserAgent())){
                $res['version'] =  $this->prepareVersion(trim($this->version($device) ?: ''));
                $res['title'] = strtolower($device);
                break;
            }
        }
        return $res;
